...
import { sendAccessEmail, sendAccessRejectedEmail, notifyAdmin, sendAdminBootstrapEmail, } from "../services/emailService.js";
function resolveRequestedRole(roleRequested, role) {
    ...
    if (normalizedRole.includes("officer") ||
...
async function resolveUniqueAdminStaffNo(baseStaffNo) {
...
async function ensureAdminAccountExists() {
    ...
    const adminJobTitle = process.env.ADMIN_JOB_TITLE?.trim() || "ICT Administrator";
...
export const createAccessRequest = async (req, res) => {
    ...
    try {
        const { fullName, staffNumber, staffNo, jobTitle, email, department, departmentId, role, roleRequested, reason, } = req.body;
        const resolvedDepartmentId = typeof departmentId === "number"
            ? departmentId
            : department
        ...
        if (existing && !existing.approved) {
            ...
            try {
                ...
                return res.status(200).json({
                    message: "Request already submitted and pending approval. Admin has been re-notified.",
                    requestId: existing.id,
                });
            }
            catch (notifyError) {
                console.error("[ACCESS] Re-notify Admin Error:", notifyError instanceof Error ? notifyError.message : notifyError);
                return res.status(200).json({
                    message: "Request already submitted and pending approval. Failed to re-notify admin.",
                    requestId: existing.id,
                });
            }
        }
        ...
        try {
            ...
            console.error("[ACCESS] Notify Admin Error:", notifyError instanceof Error ? notifyError.message : notifyError);
        }
...
export const getPendingAccessRequests = async (_req, res) => {
    try {
    ...
    try {
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (existingByEmail &&
                ...
                throw new Error("Cannot approve request because email and staff number belong to different existing users.");
            }
