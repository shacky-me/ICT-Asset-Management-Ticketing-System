...
export const createAsset = async (data, userId) => {
    const { tagNo, systemAssetId, department, category, subCategory, assetDescription, make, model, physicalCondition, serialNumber, macAddress, imeiNumber, color, departmentId, } = data;
    if (!tagNo ||
        !serialNumber ||
        !category ||
        !make ||
    ...
    const hasDepartmentId = Number.isFinite(parsedDepartmentId) && parsedDepartmentId > 0;
    ...
    const [duplicateSerial, duplicateTagNo, duplicateSystemAssetId] = await Promise.all([
...
export const removeAsset = async (id, userId) => {
    const asset = await prisma.asset.findUnique({
        where: { id },
        include: {
            assignment: {
    ...
    if (asset.assignment.length > 0) {
        throw new Error("Cannot remove asset while it has an active assignment. Return it first.");
    }
